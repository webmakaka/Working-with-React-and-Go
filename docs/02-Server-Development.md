...
```
./gomovies -dsn="host=localhost post=5432 user=postgres password=verysecret dbname=go_movies sslmode=disable"
```
...
```
[program:api]
command=env GO_MOVIES_JWT='JWT_KEY' /var/www/api.learn-code.ca/gomovies -dsn="host=localhost post=5432 user=postgres password=verysecret dbname=go_movies sslmode=disable" -env=production
directory=/var/www/api.learn-code.ca
autorestart=true
autostart=true
stdout_logfile=/var/www/api.learn-code.ca/api.log
```
